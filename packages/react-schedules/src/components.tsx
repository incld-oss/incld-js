...
export function ScheduleList({filters = {}, pageSize = 25, renderItem, onSelect, loading, empty, error, className = ''}: ScheduleListProps) {
  ...
  return <div className={`incld-list ${className}`} role="list">{state.data.data.map(schedule => <div key={schedule.id} className="incld-row" role="listitem">{renderItem ? renderItem(schedule) : <button type="button" className="incld-row-button" onClick={() => onSelect?.(schedule)}><span className="incld-row-icon" aria-hidden="true">↻</span><span className="incld-row-content"><strong>{schedule.action.displayName}</strong><small>{scheduleSummary(schedule)}</small></span><span className="incld-row-aside"><span className={`incld-badge incld-badge-${schedule.status}`}>{schedule.status}</span><small>{formatDate(schedule.nextRunAt)}</small></span></button>}</div>)}</div>;
}
...
export function RunHistory({scheduleId, filters = {}, locale, timeZone, className = '', loading, empty, error}: RunHistoryProps) {
  ...
  return <div className={`incld-history ${className}`.trim()} role="table" aria-label="Schedule run history"><div className="incld-history-header" role="row"><span>Status</span><span>Scheduled</span><span>Attempt</span></div>{state.data.data.map(run => <div className="incld-history-row" role="row" key={run.id}><span><span className={`incld-status-dot incld-status-${run.status}`} />{run.status}</span><span>{formatDate(run.nominalAt, locale, timeZone)}</span><span>{run.attemptCount}</span>{run.error && <small role="alert">{run.error}</small>}</div>)}</div>;
}
