...
export function ApprovalRequestDialog({open, onOpenChange, onCreated, className = '', ...input}: ApprovalRequestDialogProps) {
  ...
  return <IncldDialog open={open} onOpenChange={onOpenChange} className={className} title="Request approval" description="Send this change to the appropriate reviewers before it proceeds."><form className="incld-approval-request" onSubmit={submit}><div className="incld-resource-summary"><span>{input.resourceType}</span><strong>{input.title ?? input.resourceId}</strong><small>Action: {input.action}</small></div><label className="incld-field"><span>Note <small>Optional</small></span><textarea className="incld-input incld-textarea" value={note} onChange={event => setNote(event.target.value)} placeholder="Add context for reviewers" /></label>{mutation.error && <div className="incld-inline-error" role="alert">{mutation.error.message}</div>}<div className="incld-form-actions"><button type="button" className="incld-button incld-button-secondary" onClick={() => onOpenChange(false)}>Cancel</button><IncldButton type="submit" busy={mutation.pending}>Send request</IncldButton></div></form></IncldDialog>;
}
...
export function ApprovalInbox({view = 'assigned', filters = {}, pageSize = 25, renderItem, onSelect, className = '', loading, empty, error}: ApprovalInboxProps) {
  ...
  return <div className={`incld-approval-list ${className}`.trim()} role="list">{state.data.data.map(approval => <article key={approval.id} role="listitem" className="incld-approval-row">{renderItem ? renderItem(approval) : <button type="button" onClick={() => onSelect?.(approval)}><span className={`incld-approval-icon incld-approval-${approval.status}`} aria-hidden="true">{approval.status === 'approved' ? '✓' : approval.status === 'rejected' ? '×' : '?'}</span><span><strong>{approval.title ?? approval.resourceId}</strong><small>{approval.resourceType} · {approval.action}</small></span><span className="incld-approval-row-meta"><span className={`incld-badge incld-badge-${approval.status}`}>{approval.status}</span><small>{formatDate(approval.createdAt)}</small></span></button>}</article>)}</div>;
}
...
export function ApprovalTimeline({approval, className = ''}: ApprovalTimelineProps) {
  ...
  return <ol className={`incld-timeline ${className}`.trim()}>{items.map(item => <li key={item.id}><span className="incld-timeline-marker" /><div><strong>{item.type.replaceAll('.', ' ')}</strong><span>{item.actor ?? 'System'} · {formatDate(item.date)}</span>{item.reason && <p>{item.reason}</p>}</div></li>)}</ol>;
}
...
export function ApprovalDetails({approvalId, showActions = true, showTimeline = true, onResolved, className = '', loading, error}: ApprovalDetailsProps) {
  ...
  return <article className={`incld-approval-details ${className}`.trim()}><header><div><span className={`incld-badge incld-badge-${approval.status}`}>{approval.status}</span><h3>{approval.title ?? approval.resourceId}</h3><p>{approval.description ?? `${approval.resourceType} · ${approval.action}`}</p></div></header><dl><div><dt>Requested by</dt><dd>{approval.requesterId ?? 'Unknown'}</dd></div><div><dt>Requested</dt><dd>{formatDate(approval.createdAt)}</dd></div><div><dt>Policy</dt><dd>{approval.policySnapshot.mode ?? 'Default'}</dd></div><div><dt>Revision</dt><dd>{approval.revision}</dd></div></dl>{approval.status === 'pending' && showActions && <ApprovalActions approvalId={approval.id} onResolved={resolved} />}{showTimeline && <section><h4>History</h4><ApprovalTimeline approval={approval} /></section>}</article>;
}
