...
const schedule = await incld.schedules.create({
  ...
  timezone: 'Australia/Melbourne',
}, {idempotencyKey: 'schedule:user_123:sync_contacts'});
```

Never instantiate `Incld` in browser code. The key grants project-level server access.
...
The browser proxy does not expose `actions.define` or `bulkOperations.create`. Perform both with `Incld` on a trusted server.
...
| `approvalPolicies` | `list`, `get`, `create`, `update`, `remove` |
...
export const incld = createIncld({
  ...
  async resolveContext(request) {
    ...
    return {
      ...
      organization: session.organizationId ? {id: session.organizationId} : undefined,
...
approval_policies.read | approval_policies.create
approval_policies.update | approval_policies.delete
audit.read | audit.create
bulk.read | bulk.cancel
sessions.create
```

The proxy recursively removes browser-supplied `external_user_id`, `external_organization_id`, `requester_id`, `approver_id`, `actor_id`, `viewer_id`, `user_id`, and `organization_id`. It injects user/organization identity from `resolveContext` according to the operation.

Approval lists default to the requester's view. `view: 'assigned'` scopes by approver. `view: 'all'` adds neither identity filter and should only be authorized for privileged users.

## Action delivery

The framework webhook dispatches `run.created` and `bulk.chunk` to the matching declared action. Handlers receive `{action, payload, event, request, client}`. `event` includes `idempotencyKey` and, for schedule delivery, optional `scheduleId` and `runId`. Persist the idempotency key before external side effects. A recognized delivery with no declaration returns `422 action_not_declared`.
