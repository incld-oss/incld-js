...
const label = (value: string) => value.replace(/^[^.]+\./, '').replaceAll('.', ' ').replaceAll('_', ' ');
...
export function AuditTimeline({filters = {}, pageSize = 25, onSelect, renderItem, className = '', loading, empty, error}: AuditTimelineProps) {
  ...
  return <ol className={`incld-audit-timeline ${className}`}>{state.data.data.map(event => <li key={event.id}>{renderItem ? renderItem(event) : <button type="button" onClick={() => onSelect?.(event)}><span className="incld-audit-content"><strong>{label(event.type)}</strong><small>{event.actorId ?? 'System'}</small></span><time dateTime={event.occurredAt}>{formatDate(event.occurredAt)}</time></button>}</li>)}</ol>;
}
...
export function AuditEventDetails({eventId, event: supplied, className = '', loading, error}: AuditEventDetailsProps) {
  ...
  return <article className={`incld-audit-details ${className}`.trim()}><header><h3>{label(event.type)}</h3><p>{formatDate(event.occurredAt)}</p></header><dl><div><dt>Actor</dt><dd>{event.actorId ?? 'System'}</dd></div>{event.subjectType && <div><dt>Subject</dt><dd>{event.subjectId ?? event.subjectType}</dd></div>}</dl></article>;
}
