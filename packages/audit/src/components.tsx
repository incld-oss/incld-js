...
export function AuditTimeline({filters = {}, pageSize = 25, onSelect, renderItem, className = '', loading, empty, error}: AuditTimelineProps) {
  ...
  return <ol className={`incld-audit-timeline ${className}`}>{state.data.data.map(event => <li key={event.id}>{renderItem ? renderItem(event) : <button type="button" onClick={() => onSelect?.(event)}><span className="incld-audit-marker" aria-hidden="true" /><span className="incld-audit-content"><strong>{label(event.type)}</strong><small>{event.actorId ?? 'System'} · {event.component}</small></span><time dateTime={event.occurredAt}>{formatDate(event.occurredAt)}</time></button>}</li>)}</ol>;
}
...
export function AuditEventDetails({eventId, event: supplied, className = '', loading, error}: AuditEventDetailsProps) {
  ...
  return <article className={`incld-audit-details ${className}`.trim()}><header><span className="incld-badge">{event.component}</span><h3>{label(event.type)}</h3><p>{formatDate(event.occurredAt)}</p></header><dl><div><dt>Actor</dt><dd>{event.actorId ?? 'System'}</dd></div><div><dt>Source</dt><dd>{event.source}</dd></div><div><dt>Subject</dt><dd>{event.subjectType ? `${event.subjectType} / ${event.subjectId}` : '—'}</dd></div><div><dt>Visibility</dt><dd>{event.visibility}</dd></div></dl><details><summary>Event data</summary><pre>{JSON.stringify(event.data, null, 2)}</pre></details></article>;
}
